...
from modules.layers import (
    LogUniformPruningLayer,
    LogUniformL2NormPruningLayer,
    LogUniformApproximateDiracBMRPruningLayer,
    LogUniformCDFBMRPruningLayer
)
from modules.layers import (
...
def process_data(data, batch_size=32, shuffle=False, flatten=True, resize=32):
    ...
    loader = DataLoader(dset, shuffle=shuffle, batch_size=batch_size)
    ...
    with tqdm(range(n_epochs), desc=tqdm_desc) as pbar:
        for e in pbar:
            ...
            for X, Y in tqdm(loader):
                ...
                loss = recon + kl_loss
                #loss, recon, kl_loss = model.calculate_loss(criterion, logits, Y, return_all=True)
                wandb.log({'loss': loss, 'loglikelihood': recon, 'KL': kl_loss})
                loss.backward()
                optim.step()
                if scheduler:
                    scheduler.step()
            ...
            comp_pct = compress(model, set_mask=compress_epochs > 0 and (((e + 1) % compress_epochs) == 0), mu_p_tilde=mu_p_tilde)
            ...
            if acc > best_acc:
                torch.save(model.state_dict(), f'{output_dir}/{dataset}_{network}_{seed}.pth')
                best_acc = acc
...
def get_transforms(network, flatten=False, convert=False):
    if flatten:
    ...
    if network in ['lenet5', 'mlp']:
        _train_transform = T.Compose(
            [T.ToTensor(),
             ...
             fl])
        _val_transform = _train_transform
    ...
    def train_transform(examples):
        examples["X"] = [_train_transform(image.convert("RGB")) for image in examples['image']]
        return examples

    def val_transform(examples):
        examples["X"] = [_val_transform(image.convert("RGB")) for image in examples['image']]
        return examples

    if not convert:
        return _train_transform, _val_transform
    else:
        return train_transform, val_transform


def collate_fn(batch):
    X = torch.vstack([b['X'].unsqueeze(0) for b in batch])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--pruning_class", help="Name of the pruning class to use", default='LogUniformPruningLayer',
                        choices=['LogUniformPruningLayer', 'LogUniformL2NormPruningLayer',
                                 'LogUniformApproximateDiracBMRPruningLayer', 'LogUniformCDFBMRPruningLayer',], type=str)
    ...
    parser.add_argument("--dataset", help="Name of the network to use", default='mnist',
                        choices=['mnist', 'cifar10', 'fashion_mnist', 'tinyimagenet'], type=str)
    ...
    parser.add_argument("--compress_epochs", help="Number of epochs to wait to compress", type=int, default=1)
    ...
    parser.add_argument("--skip_exists", help="Exit if metrics already exist for this run", action="store_true")
    parser.add_argument("--tags", help="Tags to pass to wandb", required=False, type=str, default=[], nargs='+')
    parser.add_argument("--compression_percentage_file", help="A metrics file to pull the compression percentage from. Only for SNR and magnitude pruning", type=str,
                        required=False, default=None)

    args = parser.parse_args()

    if args.skip_exists and os.path.exists(f"{args.metrics_dir}/{args.seed}.json"):
        exit()

    comp_pct = None
    if args.compression_percentage_file != None:
        assert args.pruning_class in ['LogUniformPruningLayer', 'LogUniformL2NormPruningLayer'] and os.path.exists(args.compression_percentage_file)
        with open(args.compression_percentage_file) as f:
            metrics = json.loads(f.read())
            comp_pct = metrics['compression']
    print(comp_pct)
    # Always first
    seed = args.seed
    ...
    pruning_class = eval(args.pruning_class)
    n_layers = args.n_layers
    hidden_dim = args.hidden_dim
    compress_epochs = args.compress_epochs
    ...
    config = {
        ...
        "seed": seed,
        "use_pruning_layers": use_pruning_layers,
        "kl_averaging": kl_averaging,
    ...
    if 'mnist' in dataset:
        ...
        train_data = [d for d in mnist['train']]  # if d['label'] in [3,7]]
        ...
        test_data = [d for d in mnist['test']]  # if d['label'] in [3,7]]
        ...
        train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size,
                                                   num_workers=2, sampler=SubsetRandomSampler(indices=train_idx))
        ...
        tinyimagenet = load_dataset('zh-plus/tiny-imagenet')
        train_transform, val_transform = get_transforms(network, flatten, convert=True)
        train_indices = list(range(len(tinyimagenet['train'])))
        train_indices, dev_indices = train_test_split(train_indices, test_size=0.2,
                                                      stratify=tinyimagenet['train']['label'])

        train_dataset = Dataset.from_dict(tinyimagenet['train'][train_indices])
        valid_dataset = Dataset.from_dict(tinyimagenet['train'][dev_indices])
        train_dataset.set_transform(train_transform)
        valid_dataset.set_transform(val_transform)
        
        test_dataset = tinyimagenet['valid']
    ...
    model,val_acc,top5_acc = train_loop(model, train_loader, optim, epochs, criterion, tracker, compress_epochs=compress_epochs, kl_averaging=kl_averaging, tqdm_desc="Train", output_dir=args.output_dir, scheduler=scheduler, mu_p_tilde=mu_p_tilde)
    # Compress
    thresh = None
    if comp_pct != None:
        thresh = get_threshold(model, comp_pct)

    pct = compress(model, set_mask=True, mu_p_tilde=mu_p_tilde, thresh=thresh)
    param_pct = get_true_parameter_compression_percent(model)
    torch.save(model.state_dict(), f'{args.output_dir}/{dataset}_{network}_{seed}.pth')
    print(f"Compression %: {pct}")

    wandb.log({"Compression %": pct, 'true_param_comp': param_pct})
    metrics['compression'] = pct

    # Now train a few more epochs to retrain compressed model
    # Reset scheduler for ViT
    if network == 'vit':
        optim = AdamW(optimizer_grouped_parameters, lr=lr)
        scheduler = get_linear_schedule_with_warmup(
            optim,
            200,
            10 * len(train_loader)  # total
        )
    model,val_acc,val_top5 = train_loop(model, train_loader, optim, 10, criterion, tracker, compress_epochs=-1, kl_averaging=kl_averaging, tqdm_desc="Fine-tune", output_dir=args.output_dir, scheduler=scheduler, mu_p_tilde=mu_p_tilde)
    metrics['val_acc'] = val_acc
    metrics['val_top5_acc'] = val_top5
    tracker.stop()


    model.load_state_dict(torch.load(f'{args.output_dir}/{dataset}_{network}_{seed}.pth'))


    acc, top5_acc = evaluate(model, test_loader)
    print(acc, top5_acc)
    wandb.log({'Accuracy': acc, 'Top5_Accuracy': top5_acc})
    metrics['test_acc'] = acc
    metrics['test_top5_acc'] = acc
